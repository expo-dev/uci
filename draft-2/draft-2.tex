...
\minor A message that does not contain any tokens (that is, an empty sequence
or a sequence of one or more repetitions of \hex{20}) is \term{void}.
...
A client message has one of eight types indicated by the first token of the
message (\s{uci}, \s{debug}, \s{setoption}, \s{ucinewgame}, \s{position},
\s{isready}, \s{go}, and \s{stop}).

\major{\bf Engine Messages}

An engine message has one of six types indicated by the first token of the
message (\s{id}, \s{option}, \s{uciok}, \s{info}, \s{readyok}, \s{bestmove}).
...
% Empty messages may always be sent and must be ignored by the client
% and engine. Empty messages never cause a transition.
...
% This specification stops governing the interaction between the client
% and the engine when the client sends a well-formed quit message in
% the idle state. Engines are recommended to shut down and terminate
% themselves. Clients are recommended to provide a grace period before
% killing the engine.
