...
% TODO when the remainder of a message is viewed as bytes, are both leading
...
moves, but this condition does not cause a position to not be valid (as defined
above).}
\vskip-\parskip
...
For such positions, engines should report the null move as the best move (see
\the\majorcount·{\advance\minorcount by 7 \the\minorcount} below).}
...
\minor {\caps todo}\hem a client message has one of seven types indicated
by the first token of the message (\s{debug}, \s{setoption}, \s{ucinewgame},
\s{position}, \s{isready}, \s{go}, and \s{stop})

\major{\bf Engine Messages}

\minor {\caps todo}\hem an engine message has one of six types indicated
by the first token of the message (\s{id}, \s{option}, \s{uciok}, \s{info},
\s{readyok}, \s{bestmove})
...
for a particular row, the action listed in the row (dequeuing or enqueuing a
message of a particular type) causes a transition to the listed state.
...
{\caps todo}\hem pondering

\minor {\caps todo}\hem clients are required to send \s{ucinewgame} at least
once before the very first time they send \s{go} (but engines are strongly
recommended to behave identically whether or not \s{ucinewgame} is ever sent
or not)

\minor {\caps todo}\hem clients are required to send \s{position} at least
once before the very first time they send \s{go} (but engines are strongly
recommended to behave as if \s{position startpos} were sent if \s{position}
is not sent before \s{go} is sent)

{\caps todo}\hem note that the client is not required to send \s{isready}
before \s{go}

{\caps todo}\hem note that \s{ucinewgame} does not reset the current board
position nor does it reset the move history (only \s{position} changes the
current board state and move history)

{\caps todo}\hem note that the client may send \s{ucinewgame} after \s{position}
before \s{go} without any additional intervening \s{position}

{\caps todo}\hem note that engines and clients are not required to ignore
or otherwise handle unknown or unexpected tokens (these are simply violations)
