...
is one or more tokens (none of which are \s{value}) matching an option name
listed in an \s{option} message enqueued by the engine, and a suffix satisfying
the condition(s) for the corresponding option type:
...
or more tokens (none of which are \s{value}) matching an option name listed in
an \s{option} message enqueued by the engine for which the corresponding option
type is \s{button}.
