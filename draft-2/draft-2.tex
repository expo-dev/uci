...
\def\subsize{8}%
\font\subrm=\ldfont {\subfamily}{\subsize}{\basefeat}%
\font\subit=\ldifont{\subfamily}{\subsize}{\basefeat}%
%
\def\sub#1{\lower 0.25ex\hbox{\subrm #1}}%
\def\isub#1{\lower 0.25ex\hbox{\subit #1}}%
%
...
\minor A \term{move} is a tuple of three values: a board index called the
...
\minor A \s{setoption} message is well-formed if it has a prefix
of the form ⟨\ot\s{setoption name} {\it tok}+ \s{value}\ot⟩, where {\it tok}+ is
one or more tokens (none of which are \s{value}) matching an option name listed
in an \s{option} message enqueued by the engine, and a suffix satisfying the
...
form ⟨\ot\s{setoption name} {\it tok}+\pt⟩ where {\it tok}+ is one or more tokens
(none of which are \s{value}) matching an option name listed in an \s{option}
message enqueued by the engine for which the corresponding option type is
\s{button}.

\minor A \s{position} message is well-formed if it has the form\par
\vskip-\parskip\hskip3ex
⟨\pt\s{position}
  (\ot\s{startpos} \alt\ \s{fen} {\it fen}\ot⁶\pt)
  (\ot\s{moves} {\it mov}+\ot)?\pt\ot⟩\par
\vskip-\parskip where {\it fen}\ot⁶ is an {\caps fen} record and {\it mov}+ is
one or more move tokens. If~\s{fen} is specified, the position described by
{\it fen}\ot⁶ must be valid. If \s{moves} is specified:

{\leftskip=3ex

\vskip-0.5\parskip Let {\it P}₀ be the starting position if \s{startpos} is
specified and the position described by {\it fen}\ot⁶ if \s{fen} is specified.
Let {\it N} be the number of move tokens specified.

\vskip-0.5\parskip For {\it n} from 0 to {\it N}−1 inclusive, the {\it n}-th
token {\it T}\isub{n} of {\it mov}+ must denote a move {\it M}\isub{n} from
{\it P}\isub{n}. Then let {\it P}\sub{{\subit n}+1} be the position that
immediately follows from {\it P}\isub{n} when {\it M}\isub{n} is applied.

}

\minor A \s{go} message is well-formed if\ellipsis
...
before it enqueues a \s{go} message for the first time.
\vskip-\parskip
...
enqueues a \s{go} message for the first time.
\vskip-\parskip
