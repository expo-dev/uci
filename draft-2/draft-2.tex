...
is well-formed if it contains a single token.

\minor A \s{debug} message is well-formed if it contains two tokens,
either ⟨\s{debug on}⟩ or ⟨\s{debug off}⟩.
\vskip-\parskip
...
of the form ⟨\s{setoption name} \ellipsis\ \s{value}⟩, where the ellipsis is
...
\vskip-0.5\parskip If \s{check}, the suffix contains a single token, either
\s{true} or \s{false}.
...
form ⟨\s{setoption name} \ellipsis\pt⟩ where the ellipsis is one or more tokens
...
\minor A \s{position} message is well-formed if\ellipsis
